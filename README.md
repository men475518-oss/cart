...
- **8 つのファンタジーコース**（草原・ビーチ・雪山・火山・夜の街・1 周ごとに場所が変わる遺跡・しかけだらけの工場・宇宙のレインボーロード）
- **コースのしかけ 7 種**（転がる丸太 / 開閉する門 / ふりこ / まきあげ / 送風機 / ダッシュリング / ドッスン）
- **落ちるコース**: レインボーロードは手すりがなく、ふちを越えると落下 → 少し前の地点に戻される
- **10 種類のアイテム**（バナナ / 緑&赤のこうら / キノコ / スター / いなずま / ボム / ブーメラン / スーパーホーン / ゴールデンキノコ）
- **グランプリ**（きのこカップ 4 戦 / スターカップ 4 戦、順位ポイントの合計で総合優勝）と**タイムアタック**（コースごとのベスト記録を保存）
...
```bash
npm test             # ユニットテスト（コース生成・物理・AI・アイテム抽選・しかけ・声・BGM・カップ・対戦サーバー）
...
以下は実ブラウザで実際に測って確かめるチェックです（いずれも `npx vite preview --port 4173` が必要）。

```bash
npm run check:touch    # タッチ操作（ハンドル・ボタンの位置と反応）
npm run check:drive    # 実際に走らせて操作感・AI・ミニターボを確認
npm run check:voice    # 8 体の鳴き声がちゃんと別々になっているか（音を合成して周波数で判定）
npm run check:audio    # エンジン音 / 効果音 / BGM の音量バランス（書き出して RMS を測定）
...
（`pkill -f 'vite preview'` はパターンが自分自身のシェルにも一致して巻きこみます）。
